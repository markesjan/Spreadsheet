...
MulExpr::MulExpr(ExprPtr lhs, ExprPtr rhs) : BinaryOpExpr(std::move(lhs), std::move(rhs), "*") {}

CellData MulExpr::eval(std::map<Position, CellValue>& table) const {
...
ExprPtr MulExpr::clone() const {
	return std::make_shared<MulExpr>(lhs->clone(), rhs->clone());
}
